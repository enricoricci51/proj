...
void BoardInit(void)
{
	...
	//-------------------------------------------------------------------------
	ANSEL = 0x00;
	ANSELH = 0x02; // only AN9 is analogue input, this is the PIR_OUT

	// enable interrupt on change
	//IOCB |= 0x30;

    result |= InitBatteryMng();
	// init buttons and leds
	result |= BSP_InitializeThings();

	// initialze the temperature sensor
	result |= TCN75A_Initialize();
        
        PIR_Initialize();
        PIR_InterruptEnable(TRUE);
        PIR_PowerEnable(TRUE);

	// check for errors
	while(result)
        {
		// get here if there was an initialization error(s)
		BSP_SetLED(LED1, LED_TOGGLE);
		BSP_SetLED(LED2, LED_TOGGLE);
...
//            if(InitOk)
//            {
//                BatteryStatus = BATTERY_KO;
//
//                if(BatteryStatus == BATTERY_KO)
//                {
//                    MiApp_FlushTx();
//                    MiApp_WriteData(AL_CMD_BATTERY);
//                    MiApp_WriteData(BATTERY_KO);
