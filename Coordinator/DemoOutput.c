...
void DemoOutput_HandleMessage(void)
{
    ...
    ConsoleFillBuffer((BYTE) (CheckSum = rxMessage.Payload[i++]));
    ConsoleFillBuffer((BYTE) (rxMessage.PayloadSize - 1));
    CheckSum += (rxMessage.PayloadSize - 1);
    for(i = 1; i < rxMessage.PayloadSize; i++)
    {
        ConsoleFillBuffer(rxMessage.Payload[i]);
        CheckSum += rxMessage.Payload[i];
    }
