...
#include "p2p_demo.h"
#include "star_demo.h"
#include "system_config/miwikit_pic18f46j50_89xa/miwi_config.h"
...
bool freezer_feature(void)
{
    ...
    void dev_info(void)
    {
        ...
        if (PUSH_BUTTON_1 == 0 && PUSH_BUTTON_2 == 0)
        {
            ...
            #else
                sprintf((char *)&(LCDText), (char*)" Ch:%02d",myChannel);
            #endif

            sprintf((char *)&(LCDText[16]), (char*)"Address: %02x%02x%02x", myLongAddress[0], myLongAddress[1],myLongAddress[2]);
            LCD_Update();

            while(!dev_info_exit)
            { 
                // If a peer device is not available on Connection Table on pressing two buttons 
                // We resend the connection requests , This is required if you have a large no of
                // devices in Network Range
                #if defined(PROTOCOL_P2P) && (CONNECTION_SIZE > 4)
                    MiApp_EstablishConnection(0xFF, CONN_MODE_DIRECT);
                #endif
                #if !defined(EIGHT_BIT_WIRELESS_BOARD)
                    button_press = BUTTON_Pressed();
                #else
                    uint8_t button_press = ButtonPressed();
                #endif
