...
// Custom MAC Address if MAC_EEPROM is not used.
...
uint8_t Total_Connections(void)
{
    ...
    { 
        uint8_t i , j , k;
        uint8_t connections_perpacket = 0;
...
void P2PTasks(void)
{
    ...
    { 
        bool FW_Stat = false;  // Used for SW_Generated ACK T PAN CO
...
END_OF_SENDING_INDIRECT_MESSAGE:
        ...
        #else
            if( rxMessage.PayloadSize == 0  || P2PStatus.bits.Resync )
            {
                P2PStatus.bits.RxHasUserData = 0;
                MiMAC_DiscardPacket();
            }   
...
bool  isSameAddress(INPUT uint8_t *Address1, INPUT uint8_t *Address2)
{
    ...
    {
        uint8_t    tmpConnectionMode = ConnMode;   // Direct Mode any device in the radio range
        uint8_t    retry = CONNECTION_RETRY_TIMES;   // 3 
        uint8_t    connectionInterval = 0;   
        MIWI_TICK    t1, t2;
        tick1 = MiWi_TickGet(); 
        if( Mode == CONN_MODE_INDIRECT )
        {
            return 0xFF;        // Not in our case
        }
       
        t1 = MiWi_TickGet();

        t1.Val -= (ONE_SECOND);
        ConnMode = ENABLE_ALL_CONN;   // 0x00 
        ...
        while( i < 32 )
        {
            if( ChannelMap & FULL_CHANNEL_MAP & (channelMask << i) )
            {
               ...
               // PrintDec(i);
                ...
                while(1)
                {
                    if( MiApp_MessageAvailable(false))
                    {
                        MiApp_DiscardMessage();
                    }                     
                    //P2PTasks();
