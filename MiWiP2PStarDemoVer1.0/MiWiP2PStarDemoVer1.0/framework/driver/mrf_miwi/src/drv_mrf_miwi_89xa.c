...
#include "driver/mrf_miwi/drv_mrf_miwi_security.h"
    ...
    void RegisterSet(uint16_t setting)
    {
        ...
        PHY_IRQ1_En = 0;
        Config_nCS = 0;
        SPI1_Exchange8bit((uint8_t)(setting >> 8));   // MCC_SPI
        SPI1_Exchange8bit((uint8_t)setting);        // MCC_SPI
        Config_nCS = 1;
    ...
    uint8_t RegisterRead(uint8_t address)
    {
        ...
        SPI1_Exchange8bit(address);   // MCC_SPI
        value = SPI1_Exchange8bit(0x00);  // MCC_SPI
        Config_nCS = 1;
        PHY_IRQ1_En = IRQ1select;
    ...
    void WriteFIFO(uint8_t Data)
    {
        ...
        PHY_IRQ1_En = 0;
        Data_nCS = 0;
        SPI1_Exchange8bit(Data);    // MCC_SPI
        Data_nCS = 1;
        PHY_IRQ1_En = IRQ1select;
...
#endif
{
    if( PHY_IRQ1 && PHY_IRQ1_En )
    {
        if(RF_Mode == RF_RECEIVER)
        {
            ...
            Data_nCS = 0;
            //PacketLen = SPIGet();
            PacketLen = SPI1_Exchange8bit(0x00);             // MCC_SPI
            Data_nCS = 1;
...
IGNORE_HERE:       
                {
                    ...
                    while(fifo_stat & 0x02)
                    {
                        Data_nCS = 0;
                        //SPIGet();
                        SPI1_Exchange8bit(0x00);             // MCC_SPI
                        Data_nCS = 1;
            ...
            while(counter >= 1)
            {
                ...
                if(1)
                {
                    ...
                    if( bAck )
                    {
                        ...
                        ackPacket[RxPacketPtr++] = SPI1_Exchange8bit(0x00);   //MCC_SPI
                        Data_nCS = 1;
                    }
                    //else use the bank
                    else
                    {
                        ...
                        RxPacket[BankIndex].Payload[RxPacketPtr++] = SPI1_Exchange8bit(0x00);   // MCC_SPI
                        Data_nCS = 1;
                    }
